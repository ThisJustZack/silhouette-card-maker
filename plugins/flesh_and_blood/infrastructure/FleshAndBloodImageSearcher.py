...
class FleshAndBloodImageSearcher(ImageSearcherLike[FleshAndBloodCard]):
    async def find_image(self, card: FleshAndBloodCard, face: CardFace) -> Optional[CardImage]:
        ...
        if request_response != None:
            ...
            card_faces = card_from_response.get('faces', [{}, {}])
            match face:
                case CardFace.FRONT:
                    if len(card_faces) < 1: return None
                case CardFace.BACK: 
                    if len(card_faces) < 2: return None

            image_url = card_faces[1 if face == CardFace.BACK else 0].get('image', {}).get('large', None)
            ...
            if image_url != None:
                ...
                return CardImage(filename = DEFAULT_IMAGE_CACHE_PATH.format(CARD_ID=card.id),
                                ...
                                data = card_image.content) if card_image != None else None
